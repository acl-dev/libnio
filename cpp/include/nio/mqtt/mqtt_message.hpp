...
public:
	 ...
	 */
	virtual bool to_string(std::string& out) = 0;
...
protected:
	...
	void pack_add(unsigned char ch, std::string& out);

	// add two bytes to string buffer when building mqtt message.
	void pack_add(unsigned short n, std::string& out);

	// add string data to string buffer when building mqtt message.
	void pack_add(const std::string& s, std::string& out);
