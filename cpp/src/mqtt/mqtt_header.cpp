...
mqtt_header& mqtt_header::set_type(mqtt_type_t type) {
    if (type > MQTT_RESERVED_MIN && type < MQTT_RESERVED_MAX) {
        ...
        nio_msg_error("%s(%d): invalid type=%d", __FUNCTION__, __LINE__, (int) type);
    }
    return *this;
}
...
bool mqtt_header::build_header(std::string& out) {
    size_t len = 0;
    char header[5];
    ...
    if (dlen_ < 128) {
        ...
        nio_msg_error("%s(%d): invalid dlen_=%u", __FUNCTION__, __LINE__, dlen_);
        return false;
    }
...
int mqtt_header::update_header_len(const char* data, int dlen) {
    ...
    if (hlen_ >= sizeof(hbuf_)) {
        nio_msg_error("%s(%d): invalid header len", __FUNCTION__, __LINE__);
        return -1;
    }
