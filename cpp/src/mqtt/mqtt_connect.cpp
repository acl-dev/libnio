...
void mqtt_connect::set_will_qos(mqtt_qos_t qos) {
    ...
    case MQTT_QOS2:
        ...
        break;
    default:
        nio_msg_error("%s(%d): invalid qos: %d", __FUNCTION__, __LINE__, (int) qos);
        break;
    }
}
...
bool mqtt_connect::to_string(std::string& out) {
...
int mqtt_connect::update_header_var(const char* data, int dlen) {
    ...
    if (strcasecmp(label, "MQTT") != 0) {
        nio_msg_warn("%s(%d): invalid label: %s", __FUNCTION__, __LINE__, label);
    }
    conn_flags_ = buff_[7];

    if (!this->unpack_short(&buff_[9], 2, keep_alive_)) {
        nio_msg_error("%s(%d): unpack keep_alive error", __FUNCTION__, __LINE__);
        return -1;
    }
...
#define SAVE_DATA(buff) do { \
    if (dlen_ <= 0) { \
        nio_msg_error("%s(%d): invalid dlen_=%d", __FUNCTION__, __LINE__, dlen_); \
        return -1; \
    } \
    assert(data && dlen > 0); \
    for (; dlen_ > 0 && dlen > 0;) { \
